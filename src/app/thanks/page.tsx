...
function ThanksInner() {
  ...
  async function downloadFile(filename: string) {
    ...
    try {
      ...
      if (!r.ok) {
        const j = await r.json().catch(() => ({}))
        alert(j.error || 'Download failed. Please email enquiriesgcsemath@yahoo.com')
        return
      }
      ...
      <section style={{ maxWidth: 760, margin: '0 auto', padding: 'clamp(32px, 5vw, 48px) 20px' }}>
        ...
        {state === 'error' && (
          <div style={{ background: 'var(--burgundy-soft)', border: '1px solid var(--burgundy)', borderRadius: 12, padding: 24 }}>
            ...
            <p style={{ fontSize: 14, color: 'var(--ink-2)', lineHeight: 1.65 }}>
              Email <a href="mailto:enquiriesgcsemath@yahoo.com" style={{ color: 'var(--green)', textDecoration: 'underline' }}>enquiriesgcsemath@yahoo.com</a> with the email you used at Stripe and we&rsquo;ll send your PDFs manually.
            </p>
          </div>
        )}
